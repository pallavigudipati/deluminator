...
mem_init(void)
{
	...
	int i;
	int addr = UVPT;
	pde_t* dir = kern_pgdir;
	struct PageInfo* page_dir = pa2page(PADDR(kern_pgdir)); 
	for (i = 0; i < NPDENTRIES; ++i) {
		page_insert(kern_pgdir, page_dir, (void*)addr, PTE_U | PTE_P);
	    dir = 
	}
	*/
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	*((uint32_t*)pte_va) = pte;
	...
	// FIXME all permissions should be passed on or only few?
/*
	if (full_perm & PTE_P) {
		pgdir[PDX(va)] = pgdir[PDX(va)] | PTE_P;	
	}
...
void dump_memory(uint32_t start_va, uint32_t end_va) {
	// get the page table entry. Go to the page by dereferencing it
